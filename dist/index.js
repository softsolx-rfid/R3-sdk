...
var HexapadDriver = class extends BaseDriver {
  constructor() {
    ...
    this._port = null;
    this.subjectRaw = new import_rxjs2.Subject();
    this.messageBuffer = "";
    this.sendMessagePipe = [];
    this.pipeCron = null;
    this.cronIsRunning = false;
    // logs-related methods are implemented below
    this.logsPath = import_path.default.join(process.cwd(), "logs", "hexapad.log");
  }
  get isRunning() {
    return !!this._port;
  }
  ...
  async start() {
    try {
      ...
      this._port = new import_serialport.SerialPort({
        path: `/dev/${usbPort}`,
        baudRate: 115200,
        dataBits: 8,
        stopBits: 1,
...
`);
  ...
  async log(...data) {
    try {
      const file = await import_fs2.promises.open(this.logsPath, "a");
      await file.write(
...
`).join("")
  ...
  async createLogsFileIfNotExists() {
    const logsDir = import_path.default.dirname(this.logsPath);
    await import_fs2.promises.mkdir(logsDir, { recursive: true });
    const file = await import_fs2.promises.open(this.logsPath, "a", 438);
    await file.close();
  }
  async getLogs(maxLines = 1e3) {
    try {
      const file = await import_fs2.promises.open(this.logsPath, "r");
      const stats = await file.stat();
      const fileSize = stats.size;
      const bufferSize = Math.min(fileSize, 5 * 1024 * 1024);
      const buffer = Buffer.alloc(bufferSize);
